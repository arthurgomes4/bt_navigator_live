...
bool MonitoredNavigateThroughPosesNavigator::configure(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node,
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  // Call parent configure first to setup the action server and BT
  if (!NavigateThroughPosesNavigator::configure(parent_node, odom_smoother)) {
    return false;
  }
...
bool MonitoredNavigateThroughPosesNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
  // Call parent to load the BT
  if (!NavigateThroughPosesNavigator::goalReceived(goal)) {
    return false;
  }
...
bool MonitoredNavigateThroughPosesNavigator::cleanup()
{
  // Clean up the Groot logger
  groot_logger_.reset();
  
  // Call parent cleanup
  return NavigateThroughPosesNavigator::cleanup();
}
