...
#include <memory>
#include <limits>
#include "live_navigate_to_pose.hpp"

namespace live_navigator_plugins
{

bool LiveNavigateToPoseNavigator::configure(
  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
    "live_navigator_goal_pose",
    rclcpp::SystemDefaultsQoS(),
    std::bind(&LiveNavigateToPoseNavigator::onGoalPoseReceived, this, std::placeholders::_1));
  
  // load the tree here instead of when the goal is received
  std::string bt_xml_filename = getDefaultBTFilepath(parent_node);
  if (!bt_action_server_->loadBehaviorTree(bt_xml_filename)) {
    
    RCLCPP_ERROR(
      logger_, "BT file not found: %s. configuration canceled.",
...
std::string LiveNavigateToPoseNavigator::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  std::string default_bt_xml_filename;
  auto node = parent_node.lock();

  if (!node->has_parameter("default_live_nav_to_pose_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("live_navigator_plugins");
    node->declare_parameter<std::string>(
      "default_live_nav_to_pose_bt_xml",
      pkg_share_dir + "/config/tree_xml/navigate_to_pose_w_replanning_and_recovery.xml");
  }

  node->get_parameter("default_live_nav_to_pose_bt_xml", default_bt_xml_filename);

  return default_bt_xml_filename;
}

bool LiveNavigateToPoseNavigator::cleanup()
{
  goal_sub_.reset();
  self_client_.reset();
  return true;
...
bool LiveNavigateToPoseNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
  return initializeGoalPose(goal);
}

void
LiveNavigateToPoseNavigator::goalCompleted(
...
void
LiveNavigateToPoseNavigator::onLoop()
{
...
void
LiveNavigateToPoseNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
bool
LiveNavigateToPoseNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
  geometry_msgs::msg::PoseStamped current_pose;
  if (!nav2_util::getCurrentPose(
      current_pose, *feedback_utils_.tf,
...
void
LiveNavigateToPoseNavigator::onGoalPoseReceived(const geometry_msgs::msg::PoseStamped::SharedPtr pose)
{
  ActionT::Goal goal;
  goal.pose = *pose;
...
PLUGINLIB_EXPORT_CLASS(
  live_navigator_plugins::LiveNavigateToPoseNavigator,
