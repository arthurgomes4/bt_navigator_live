  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  odom_smoother_ = odom_smoother; // Odometry smoother object for getting current speed
...
std::string
NavigateToPoseNavigator::getBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  std::string bt_xml_filename;
  auto node = parent_node.lock();

  if (!node->has_parameter("live_navigate_to_pose_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("live_navigator_plugins");
    node->declare_parameter<std::string>(
      "live_navigate_to_pose_bt_xml",
      pkg_share_dir +
      "/config/navigate_to_pose_w_replanning_and_recovery.xml");
  }

  node->get_parameter("live_navigate_to_pose_bt_xml", bt_xml_filename);
...
{
  auto bt_xml_filename = goal->behavior_tree; // todo load behvaior tree in config function
